...
export ETH_ADDRESS="0x466d9994e17352c719359CF58C52ACCB46f447Ff"
...
exec geth --datadir /data --config config.toml --bootnodes="$BOOTNODE" --networkid $ETH_CHAIN_ID --port 30301 --rpcvhosts=*  --allow-insecure-unlock --nousb --verbosity 3 --gcmode=archive --mine --miner.threads 1 --unlock $ETH_ADDRESS --password /tmp/eth_pass --miner.gasprice 0
else
  exec "$@"
fi
