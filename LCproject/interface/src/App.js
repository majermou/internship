...
export default function App() {
    ...
    const provider = new ethers.providers.JsonRpcProvider(process.env.REACT_APP_nodeUrl);
    ...
    /*
        inspect() function to inspect an insured defined by his cine
        @param variable cine
    */
    const inspect = async () => {
        ...
        try {
            let flag = false;
            setInspecting(true);
            // set a timeout for the createRequest() smart contract call
            setTimeout(() => {
                if (!flag) {
            ...
            // get the current block number in order to capture the right emited event in useEffect
    ...
    useEffect(() => {
      if (inspecting === true) {
        ...
        // set a timeout for oracle answers
        setTimeout(() => {
        ...
        // event listener on oracle1 answer
        oracleContract.on("responseByOracle1", async (_cine, event) => {
            if (event.blockNumber >= blockNum) {
                ...
                if (prm) {
                  console.log('both oracles answered');
        ...
        // event listener on oracle2 answer
        oracleContract.on("responseByOracle2", async (_cine, event) => {
            if (event.blockNumber >= blockNum) {
                console.log("oracle2 answered");
                secondOracleFlag = !secondOracleFlag;
                // if oracle1 answer is recieved then proceed to get the data from the smart contract
                if (prm) {
                  console.log('both oracles answered');
                  const assureData = await oracleContract.getInsuredData(_cine);
                  setAssure(assureData);
